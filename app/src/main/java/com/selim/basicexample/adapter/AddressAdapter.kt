...
class AddressAdapter(val addressList:ArrayList<Address>) : RecyclerView.Adapter<AddressAdapter.AddressVH>() {
    ...
    override fun onBindViewHolder(holder: AddressVH, position: Int) {
        ...
        holder.itemView.tv_address_info.text = addressList.get(position).city + " " + addressList.get(position).district + " " +
                addressList.get(position).avenue + " " + addressList.get(position).neighborhood + " " + addressList.get(position).street + " " +
